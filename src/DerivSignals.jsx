...
function LiveCandleChart({ candles, dark, pair, tf }) {
  if (!candles || candles.length < 3) return (
    <div style={{ height:280, background:dark?"#0a1520":"#e8f4ff", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8, borderRadius:8 }}>
      <div style={{ fontSize:24 }}>📊</div>
      <div style={{ fontSize:11, color:dark?"#4499ff":"#0055aa", fontFamily:"monospace" }}>
        Connecting... {candles?.length||0} candles loaded
      </div>
    </div>
  );

  const display = [...candles].slice(0, 50).reverse();
  ...
  const W = 600, H = 260, padL = 55, padB = 20, padT = 10;
  const chartW = W - padL;
  const chartH = H - padB - padT;
  const cw = Math.max(3, (chartW / display.length) - 1);

  const toY = p => padT + ((maxP - p) / priceRange) * chartH;
  const toX = i => padL + (i / display.length) * chartW + cw / 2;

  const gridCount = 5;
  const grids = Array.from({length:gridCount+1}, (_,i) => minP + (priceRange/gridCount)*i);

  const currentPrice = candles[0]?.close;
  const decimals = pair?.startsWith("frx") ? 5 : 2;

  return (
    <div style={{ background:dark?"#0a1520":"#f0f8ff", borderRadius:8, overflow:"hidden", padding:"4px 0" }}>
      <div style={{ padding:"4px 10px", fontSize:9, color:dark?"#4499ff":"#0055aa", fontFamily:"monospace", fontWeight:700, display:"flex", justifyContent:"space-between" }}>
        <span>📊 {pair} · {tf}</span>
        <span style={{ color:"#ffd700" }}>● {currentPrice?.toFixed(decimals)}</span>
      </div>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display:"block" }}>
        {/* Background */}
        <rect x={padL} y={padT} width={chartW} height={chartH} fill={dark?"#050a0f":"#ffffff"} />

        {/* Grid */}
        {grids.map((p,i) => (
          <g key={i}>
            <line x1={padL} y1={toY(p)} x2={W} y2={toY(p)}
              stroke={dark?"#1e3040":"#e0eaf4"} strokeWidth="0.5" strokeDasharray="3,3"/>
            <text x={padL-4} y={toY(p)+3} fontSize="7" fill={dark?"#445566":"#778899"}
              textAnchor="end">{p.toFixed(decimals)}</text>
          </g>
        ))}
        ...
        {currentPrice && (
          <g>
            <line x1={padL} y1={toY(currentPrice)} x2={W} y2={toY(currentPrice)}
              stroke="#ffd700" strokeWidth="1" strokeDasharray="5,3"/>
            <rect x={W-58} y={toY(currentPrice)-9} width={58} height={16}
              fill="#ffd700" rx="3"/>
            <text x={W-29} y={toY(currentPrice)+4} fontSize="8" fill="#000"
