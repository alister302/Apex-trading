...
const PROMPT = `You are APEX FX — elite candlestick analyst. Analyze chart screenshot. ALWAYS give prediction. Return ONLY JSON:
...
const PREMIUM_TABS = ["live","elite","longterm","fast"];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: sub_ } = supabase.auth.onAuthStateChange((_, s) => setUser(s?.user || null));
    return () => sub_?.subscription?.unsubscribe();
  }, []);
  ...
  useEffect(() => {
    ...
    if (trackingId && orderId) {
      verifyPayment(trackingId, orderId);
    }
  }, [user]);
  ...
  const verifyPayment = async (trackingId, orderId) => {
    try {
      const res = await fetch(`${SERVER}/pesapal/verify`, {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ order_tracking_id:trackingId, order_id:orderId, user_id:user.id })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const verifyAdmin = async () => {
    ...
    } catch(e) { setAdminErr("Server error"); }
  };

  const handleTabClick = (tabId) => {
    if (PREMIUM_TABS.includes(tabId) && !isPremium) {
      setShowSub(true);
      return;
    }
    setTab(tabId);
  ...
  const analyze = async () => {
    ...
    try {
      const res = await fetch(GEMINI_URL, { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ contents:[{parts:[{text:PROMPT+"\nAnalyze chart."},{inline_data:{mime_type:mime,data:b64}}]}], generationConfig:{temperature:0.2,maxOutputTokens:8192,responseMimeType:"application/json"} })
      });
      const data = await res.json();
  ...
  if (!authReady) return <div style={{minHeight:"100vh",background:"#050a0f",display:"flex",alignItems:"center",justifyContent:"center",color:"#0066ff",fontFamily:"monospace",fontSize:14}}>Loading...</div>;
    ...
    <div style={{height:"100vh",background:t.bg,color:t.text,fontFamily:"'IBM Plex Mono',monospace",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .hbtn{transition:all 0.2s;cursor:pointer;border:none;font-family:'IBM Plex Mono',monospace}
      ...
      {adminPrompt && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:dark?"#030810":"#fff",border:"1px solid #0d2a42",borderRadius:14,padding:28,width:"100%",maxWidth:360}}>
            <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:13,fontWeight:900,color:"#ffd700",letterSpacing:2,marginBottom:16,textAlign:"center"}}>🔐 ADMIN ACCESS</div>
            <input type="password" value={adminKey} onChange={e=>setAdminKey(e.target.value)} onKeyDown={e=>e.key==="Enter"&&verifyAdmin()} placeholder="Enter admin passkey" style={{width:"100%",padding:"12px",background:"rgba(0,40,80,0.3)",border:"1px solid #0d2a42",borderRadius:7,color:"#c8d8e8",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,outline:"none",marginBottom:10}} />
            {adminErr && <div style={{fontSize:11,color:"#ff4466",marginBottom:10}}>⚠ {adminErr}</div>}
            <div style={{display:"flex",gap:8}}>
              <button className="hbtn" onClick={verifyAdmin} style={{flex:1,padding:"10px",background:"linear-gradient(135deg,#ffd700,#cc9900)",color:"#000",borderRadius:6,fontWeight:700,fontSize:11}}>VERIFY</button>
              <button className="hbtn" onClick={()=>{setAdminPrompt(false);setAdminKey("");setAdminErr("");}} style={{flex:1,padding:"10px",background:"transparent",border:"1px solid #0d2a42",color:t.muted,borderRadius:6,fontWeight:700,fontSize:11}}>CANCEL</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{flexShrink:0,background:t.bgH,borderBottom:`1px solid ${t.border}`,padding:"0 14px"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:50}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:28,height:28,background:"linear-gradient(135deg,#0066ff,#00aaff)",clipPath:"polygon(50% 0%,100% 25%,100% 75%,50% 100%,0% 75%,0% 25%)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{fontSize:12,color:"#fff"}}>▲</span>
            </div>
            <div>
              <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:13,fontWeight:900,color:dark?"#fff":"#0044bb",letterSpacing:3}}>APEX <span style={{color:"#ffd700"}}>FX</span></div>
              <div style={{fontSize:7,letterSpacing:2,color:t.dim}}>CANDLE INTELLIGENCE</div>
            </div>
          </div>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            {sub?.active && <span style={{fontSize:8,padding:"2px 7px",background:"#00dd5522",border:"1px solid #00dd5533",color:"#00dd55",borderRadius:4,fontWeight:700,letterSpacing:1}}>{sub.plan?.toUpperCase()}</span>}
            {isAdmin && <span style={{fontSize:8,padding:"2px 7px",background:"#ffd70022",border:"1px solid #ffd70033",color:"#ffd700",borderRadius:4,fontWeight:700}}>ADMIN</span>}
            <span style={{fontSize:8,color:"#00aa44"}}>● LIVE</span>
            <button className="hbtn" onClick={()=>setDark(!dark)} style={{background:"transparent",border:`1px solid ${t.border}`,borderRadius:5,padding:"4px 7px",fontSize:13}}>
              {dark?"☀️":"🌙"}
            </button>
            {isAdmin && <button className="hbtn" onClick={()=>setAdminPrompt(true)} style={{background:"#ffd70022",border:"1px solid #ffd70033",borderRadius:5,padding:"4px 8px",fontSize:9,color:"#ffd700",fontWeight:700}}>⚙ ADMIN</button>}
            <button className="hbtn" onClick={()=>setShowSub(!showSub)} style={{background:isPremium?"#00dd5522":"linear-gradient(135deg,#0066ff,#0044bb)",border:isPremium?"1px solid #00dd5533":"none",borderRadius:5,padding:"4px 8px",fontSize:9,color:isPremium?"#00dd55":"#fff",fontWeight:700}}>
              {isPremium?"✓ PRO":"⚡ UPGRADE"}
            </button>
            <button className="hbtn" onClick={()=>supabase.auth.signOut()} style={{background:"#ff224411",border:"1px solid #ff224433",borderRadius:5,padding:"4px 8px",fontSize:9,color:"#ff4466",fontWeight:700}}>OUT</button>
          </div>
        </div>
      </div>
        ...
        <div style={{display:"flex",minWidth:"max-content",padding:"0 4px"}}>
          {TABS.map(tb=>(
            <button key={tb.id} onClick={()=>handleTabClick(tb.id)}
              ...
              {tb.label}
              {tb.premium&&!isPremium&&<span style={{position:"absolute",top:4,right:2,fontSize:7,color:"#ffd700"}}>🔒</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
        ...
        {/* Subscription page */}
        {showSub && <Subscription user={user} sub={sub} onSubscribed={()=>{checkSubscription();setShowSub(false);}} dark={dark} />}
        ...
        {!showSub && <>
          ...
          {tab==="analyzer" && (
            <div style={{maxWidth:1100,margin:"0 auto",width:"100%",padding:"14px 14px",display:"grid",gridTemplateColumns:"1fr 260px",gap:14}}>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div style={{border:`2px dashed ${drag?"#0088ff":t.borderD}`,background:drag?(dark?"rgba(0,100,255,0.06)":"#e8f4ff"):t.inp,borderRadius:8,padding:18,textAlign:"center",minHeight:170,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,cursor:"pointer",transition:"all 0.2s"}}
                  ...
                  {image?(
                    ...
                    <>
                      ...
                      <div style={{fontSize:12,fontWeight:700,color:dark?"#4499cc":"#0055aa"}}>DROP CHART HERE</div>
                      <div style={{fontSize:10,color:t.muted}}>PNG / JPG · Any pair · Any timeframe</div>
                    </>
                  )}
                </div>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  {image&&<button onClick={()=>{setImage(null);setB64(null);setResult(null);setErr(null);}} style={{background:t.bgCard,border:`1px solid ${t.border}`,color:t.muted,padding:"14px 14px",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,cursor:"pointer",borderRadius:6,fontWeight:600}}>CLEAR</button>}
                </div>
                ...
                {loading&&<div style={{background:t.bgCard,border:`1px solid ${t.border}`,padding:18,textAlign:"center",borderRadius:8}}><div className="pulse" style={{fontSize:12,fontWeight:700,letterSpacing:2,color:"#0088ff"}}>APEX FX SCANNING CHART...</div></div>}
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:8,padding:"12px 14px"}}>
                      ...
                      <div style={{display:"flex",gap:14}}>
                        <div style={{fontSize:11,color:"#00bb55",fontWeight:600}}>▲ S: {result.support}</div>
                        <div style={{fontSize:11,color:"#ee2244",fontWeight:600}}>▼ R: {result.resistance}</div>
                      </div>
                    </div>
                    <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:8,padding:"12px 14px"}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                        {(result.patterns||[]).map((p,i)=>(
                          <span key={i} style={{background:dark?"#001a33":"#e0eeff",border:`1px solid ${dark?"#0d3a5a":"#99bbdd"}`,color:dark?"#4499cc":"#0055aa",fontSize:9,padding:"2px 8px",borderRadius:4,fontWeight:700}}>{p}</span>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div style={{fontSize:10,letterSpacing:2,color:t.label,marginBottom:8,fontWeight:700}}>NEXT 3 CANDLES</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                        {(result.candles||[]).map(c=>(
                          <div key={c.number} style={{background:t.bgCard,border:`2px solid ${dc(c.direction)}33`,borderRadius:8,padding:12,borderTop:`3px solid ${dc(c.direction)}`}}>
                            ...
                            <div style={{fontSize:20,fontWeight:900,color:dc(c.direction),fontFamily:"'Orbitron',sans-serif"}}>{c.confidence}%</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {history.length===0&&<div style={{border:`1px dashed ${t.borderD}`,borderRadius:8,padding:14,textAlign:"center",fontSize:10,color:t.dim}}>No scans yet</div>}
                {history.map((h,i)=>(
      ...
      <div style={{flexShrink:0,borderTop:`1px solid ${t.border}`,padding:"5px 14px",display:"flex",justifyContent:"space-between",background:t.bgH}}>
        <div style={{fontSize:8,color:t.dim}}>APEX FX · PRINCE X TOOLS</div>
