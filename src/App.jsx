...
export default function App() {
    ...
    try {
      const res = await fetch(GEMINI_URL, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              { text: SYSTEM_PROMPT + "\n\nAnalyze this OlympTrade chart. Return ONLY valid JSON." },
  ...
  return (
    <div style={{ height: "100vh", background: t.bg, color: t.text, fontFamily: "'IBM Plex Mono', monospace", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* FIXED TAB BAR — scrollable left/right */}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button className="scan-btn" onClick={analyze} disabled={!imageBase64 || loading} style={{ flex: 1 }}>
                  {loading ? "⟳  SCANNING..." : "⚡  RUN APEX ANALYSIS"}
                </button>
                {image && (
              ...
              {error && (
                <div style={{ background: dark ? "#2a000a" : "#fff0f3", border: "1px solid #cc0022", padding: "12px 16px", fontSize: 12, color: "#cc0022", borderRadius: 6, fontWeight: 600 }}>⚠ {error}</div>
              )}

              {loading && (
                <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, padding: 24, textAlign: "center", borderRadius: 8 }}>
                  <div className="pulse" style={{ fontSize: 13, fontWeight: 700, letterSpacing: 2, color: "#0088ff" }}>APEX SCANNING CHART...</div>
              ...
              {analysis && !loading && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 8, padding: "16px 20px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14 }}>
                    {[
                      { label: "PAIR", value: analysis.pair, color: dark ? "#ffffff" : "#001133" },
                      { label: "TIMEFRAME", value: analysis.timeframe, color: t.textMuted },
                      { label: "BIAS", value: analysis.market_bias, color: biasColor(analysis.market_bias) },
                      { label: "SIGNAL", value: analysis.entry_signal, color: signalColor(analysis.entry_signal) }
                  ...
                  <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: `4px solid ${biasColor(analysis.market_bias)}`, borderRadius: 8, padding: "14px 18px" }}>
                    <div style={{ fontSize: 9, letterSpacing: 2, color: t.textLabel, marginBottom: 8, fontWeight: 700 }}>APEX SUMMARY</div>
      ...
      <div style={{ flexShrink: 0, borderTop: `1px solid ${t.border}`, padding: "8px 20px", display: "flex", justifyContent: "space-between", background: t.bgHeader }}>
        <div style={{ fontSize: 10, color: t.textDim }}>APEX · POWERED BY PRINCE X TOOLS</div>
